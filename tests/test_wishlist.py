...
class TestWishlistRoutes:
    ...
    def test_add_item(self, client, app, user):
        ...
        response = client.post('/wishlist/add', data={
            'name': 'Test Item',
        ...
        with app.app_context():
            ...
            assert count_after > count_before, f"Item should be created in database (before: {count_before}, after: {count_after})"
    ...
    def test_edit_item(self, client, app, user, wishlist_item):
        ...
        self.login_user(client, app, user)
        response = client.post(f'/wishlist/edit/{wishlist_item.id}', data={
            'name': 'Updated Item',
    ...
    def test_cannot_edit_others_item(self, client, app, user, admin_user):
        ...
        response = client.post(f'/wishlist/edit/{item_id}', data={
            'name': 'Hacked Item',
            'quantity': '1',
        }, follow_redirects=True)
    ...
    def test_purchase_item(self, client, app, user, admin_user):
        ...
        # Purchase item
        response = client.post(f'/wishlist/purchase/{item_id}', data={
            'quantity': '1',
            'submit': 'Mark as Purchased'
        }, follow_redirects=True)
    ...
    def test_cannot_purchase_own_item(self, client, app, user, wishlist_item):
        ...
        self.login_user(client, app, user)
        response = client.post(f'/wishlist/claim/{wishlist_item.id}', data={
            'quantity': '1',
        }, follow_redirects=True)
    ...
    def test_add_empty_item(self, client, app, user):
        ...
        self.login_user(client, app, user)
        response = client.post('/wishlist/add-empty',
    ...
    def test_update_item_quick(self, client, app, user, wishlist_item):
        ...
        self.login_user(client, app, user)
        response = client.post(f'/wishlist/update-item/{wishlist_item.id}',
                             data=json.dumps({
                                 'name': 'Quick Updated',
                                 'price': '49.99'
                             }),
...
class TestWishlistChange:
    ...
    def test_wishlist_change_on_update(self, app, user, wishlist_item):
        ...
        with app.app_context():
            ...
            # Verify change
            changes = WishlistChange.query.filter_by(
                change_type='updated',
                item_id=wishlist_item.id
            ).all()
            assert len(changes) > 0
